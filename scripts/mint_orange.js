/**
 * @fileoverview Create the $ORANGE fungible token on Hedera testnet
 * @module scripts/mint_orange
 *
 * This script creates a $ORANGE token with the following properties:
...
async function createOrangeToken() {
  ...
  const metadata = { name: "Orange", symbol: "ORANGE", color: "#FFA500" };

  logger.info("Creating $ORANGE token...", {
  ...
  logger.success("$ORANGE token created", {
...
async function main() {
  try {
    logger.section("Mint $ORANGE Token");

    const result = await createOrangeToken();

    logger.subsection("Next Steps");
    logger.info("Add the following to your .env file:");
    console.log(`ORANGE_TOKEN_ID=${result.tokenId}`);
    console.log(`ORANGE_ADDR=${result.evmAddress}`);

  } catch (err) {
    logger.error("Failed to create $ORANGE token", err);
    process.exit(1);
  }
}

// Export for programmatic use
export { createOrangeToken };

// Run if executed directly
if (import.meta.url === `file:///${process.argv[1].replace(/\\/g, '/')}` ||
    import.meta.url.endsWith('mint_orange.js')) {
  main();
}
