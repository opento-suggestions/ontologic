/**
 * @fileoverview Create the $PINK fungible token on Hedera testnet
 * @module scripts/mint_pink
 *
 * This script creates a $PINK token with the following properties:
...
/**
 * Create the $PINK token on Hedera
...
async function createPinkToken() {
  ...
  const metadata = { name: "Pink", symbol: "PINK", color: "#FFC0CB" };

  logger.info("Creating $PINK token...", {
  ...
  logger.success("$PINK token created", {
...
async function main() {
  try {
    logger.section("Mint $PINK Token");

    const result = await createPinkToken();

    logger.subsection("Next Steps");
    logger.info("Add the following to your .env file:");
    console.log(`PINK_TOKEN_ID=${result.tokenId}`);
    console.log(`PINK_ADDR=${result.evmAddress}`);

  } catch (err) {
    logger.error("Failed to create $PINK token", err);
    process.exit(1);
  }
}

// Export for programmatic use
export { createPinkToken };

// Run if executed directly
if (import.meta.url === `file:///${process.argv[1].replace(/\\/g, '/')}` ||
    import.meta.url.endsWith('mint_pink.js')) {
  main();
}
